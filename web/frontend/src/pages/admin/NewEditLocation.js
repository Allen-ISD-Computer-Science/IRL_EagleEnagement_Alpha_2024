    ...
    try {
      ...
      if (res.status === 200) {
        ...
        if (isNew && eventRequest) {
          toast.info("Redirecting to create the event in 1 second.");
          setTimeout(() => {
            window.location.href = `${process.env.PUBLIC_URL}/admin/events/new?request=${eventRequest.id}`;
          }, 1000);
        }
      } else {
    ...
    <div className="flex flex-row items-stretch min-h-[100vh]">
      ...
      <div className="flex flex-col items-stretch w-full">
        ...
        <div className="flex flex-row justify-center align-center max-md:!flex-col-reverse">
          ...
          <form className=" flex-1 flex flex-row justify-between items-stretch p-10 gap-10 max-md:flex-col max-md:gap-4 max-md:p-4" onSubmit={(e) => { e.preventDefault(); }}>
            <ToastContainer
              position="top-right"
              autoClose={2000}
